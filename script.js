...
class CatCard{constructor(obj,)
    ...
    let data = [];  
    let cnt = 12;
    for(let i = 0; i < koshaki.length; i++){ let cat = koshaki[i]    
    data.push(new CatCard(cat))}
    console.log(data)
    let html = ""
    for(let val of data){  
    stars = "";
    for(let i = 0; i < Math.ceil(val.rate/2); i++){
        stars+="&#9733;"
    }
    let WhatIsLove = (val.favourite ? 'Любимчик)' : 'Не любимчик(')
    document.body.innerHTML += 
    `<div class ='card'>    
    <div class = "CatInf">  
    <h1>${val.name}</h1>   
    <h3>${val.age} годиков</h3>     
    <p>Рейтинг - ${stars}</p>  
    <br></br>     
    <p>${val.description}</p>
    <br></br> 
    ...
    const ToNorm = function(){
        this.style.backgroundColor = `#fff`
    }
for (let i of crd){
    i.addEventListener("mouseover", changeBg)
}

btn.addEventListener("click", e => {for (let i of crd){
